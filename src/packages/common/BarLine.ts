...
const normalSeries = (
  ...
  // 是否全部为正数
  const isPositiveNumber = dataSource.every(el => Number(el.value) > 0);
  if (!Array.isArray(dataSource)) {
    console.warn("dataSource 必须是数组");
    return [];
  }
  ...
  if (seriesNum.length) {
    series = seriesNum.map((item, index) => {
      ...
      dataSourceMap.data = dataSource.reduce((cur: ObjectKey[], next) => {
        if (next.series == dataSourceMap.name) {
          if (!isPositiveNumber) {
            next.value < 0
              ? (next["itemStyle"] = {
    ...
    dataSourceMap.data = dataSource.map(item => {
      if (!isPositiveNumber) {
        item.value < 0
          ? (item["itemStyle"] = {
      ...
      formatter: (param: any) => {
        if (Array.isArray(param) && param.length > 2) {
          ...
          return `${target.name}<br/>${target.marker || ""}${
            target.seriesName
          } : <b style="font-weight:bold; color: #000">${target.value}</b>`;
        }
        return "";
      }
    };
